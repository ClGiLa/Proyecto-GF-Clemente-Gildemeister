...
Además, el crecimiento de Nvidia en 2023 ha sido impulsado por la demanda global de infraestructura de IA, con sus procesadores siendo adoptados por gigantes tecnológicos como OpenAI y Google, lo que refuerza la relevancia del sector tecnológico en mercados como el Nasdaq (El Sol de México) (El País).
...
data = yf.download(sp500_top10, start="2023-01-01", end="2024-09-01", group_by='ticker')
```
...
Nvidia Corporation es uno de los principales actores del sector tecnológico en el Nasdaq, particularmente en el desarrollo de inteligencia artificial (IA) y procesadores gráficos (GPU). Nvidia se encuentra junto a líderes tecnológicos como Apple, Microsoft y Google en el Nasdaq. Para analizar las tendencias del sector y la posición competitiva de Nvidia, se presentan los precios históricos de Nvidia y sus principales rivales en el Nasdaq, como AMD e Intel: 

```{python}
# Definir el rango de fechas
end = data.index.max()  # Última fecha en el DataFrame
start = end - timedelta(weeks=40)  # Hace 40 semanas

# Filtrar los datos dentro del rango de fechas
filtered_data = data.loc[start:end]

# Seleccionar las columnas de precios de cierre ('Close') para cada empresa
close_columns = [col for col in data.columns if col[1] == 'Close']
filtered_data_close = filtered_data[close_columns]

# Suavizar las líneas con medias móviles
window_size = 7  # Puedes ajustar este valor para más o menos suavizado
smoothed_data = filtered_data_close.rolling(window=window_size, min_periods=1).mean()

# Crear el gráfico
plt.figure(figsize=(12, 8))

# Dibujar un gráfico de líneas suavizadas para cada empresa
for col in smoothed_data.columns:
    plt.plot(smoothed_data.index, smoothed_data[col], label=col[0])  # col[0] es el símbolo de la empresa

# Configurar etiquetas y título
plt.title('Comparación de Precios de Cierre para las 10 Mejores Empresas del S&P 500 \n (Suavisacion mediante medias moviles)', fontsize=16)
...
# Supongamos que ENELCHILE es un DataFrame de pandas con una columna 'close'
# ENELCHILE = pd.read_csv('ruta_al_archivo.csv')  # Cargar los datos si es necesario

# Cálculo de rendimientos y volatilidad
rendimientos = np.diff(np.log(ENELCHILE['close']))
sigma = np.std(rendimientos[1:])  # Volatilidad
volatilidad = np.var(rendimientos[1:])  # Varianza

# Parámetros de la opción
S = ENELCHILE['close'].iloc[-1]  # Precio actual del activo subyacente
K = 38  # Precio de ejercicio de la opción
R = 0.0642  # Tasa de interés libre de riesgo
t = 1  # Tiempo hasta la expiración (en años)
